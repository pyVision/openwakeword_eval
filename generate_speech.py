...
requiredNamed.add_argument('--n_speakers', metavar='', type=str, help="""number of speakers.""", default="")
...
if args.model == "VITS":
    ...
    if args.input_file == "":
        ids = generate_speaker_ids(args.N, n_speakers=n_speakers, n_mix=5)  #for VITS model trained on VCTK dataset
        texts = []
        ...
        ids = generate_speaker_ids(len(texts), n_speakers=n_speakers, n_mix=5)  #for VITS model trained on VCTK dataset
    ...
    # Get speaker ids and text for each generation
    if args.input_file == "":
        ids = generate_speaker_ids(args.N, n_speakers=n_speakers, n_mix=2) # for waveglow model trained on LibriTTS dataset
        texts = []
        for i in range(len(ids)):
            text = re.sub("<any>", lambda x: random.choice(en_words), args.text)
            if "|" in text:
        ...
        with open(args.input_file, 'r') as f:
            texts = f.readlines()
            texts = [i.strip() for i in texts]
        ids = generate_speaker_ids(len(texts), n_speakers=n_speakers, n_mix=2) # for waveglow model trained on LibriTTS dataset
